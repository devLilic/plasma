import React from 'react';

const SearchImageIcon = () => {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
             width="35" height="25"
             viewBox="0 0 100 100">
            <path fill="#ee3e54" d="M13 27A2 2 0 1 0 13 31A2 2 0 1 0 13 27Z"></path>
            <path fill="#f1bc19" d="M77 12A1 1 0 1 0 77 14A1 1 0 1 0 77 12Z"></path>
            <path fill="#fce0a2" d="M50 13A37 37 0 1 0 50 87A37 37 0 1 0 50 13Z"></path>
            <path fill="#f1bc19" d="M83 11A4 4 0 1 0 83 19A4 4 0 1 0 83 11Z"></path>
            <path fill="#ee3e54" d="M87 22A2 2 0 1 0 87 26A2 2 0 1 0 87 22Z"></path>
            <path fill="#fbcd59"
                  d="M81 74A2 2 0 1 0 81 78 2 2 0 1 0 81 74zM15 59A4 4 0 1 0 15 67 4 4 0 1 0 15 59z"></path>
            <path fill="#ee3e54" d="M25 85A2 2 0 1 0 25 89A2 2 0 1 0 25 85Z"></path>
            <path fill="#fff" d="M18.5 51A2.5 2.5 0 1 0 18.5 56A2.5 2.5 0 1 0 18.5 51Z"></path>
            <path fill="#f1bc19" d="M21 66A1 1 0 1 0 21 68A1 1 0 1 0 21 66Z"></path>
            <path fill="#fff" d="M80 33A1 1 0 1 0 80 35A1 1 0 1 0 80 33Z"></path>
            <path fill="#48bed8"
                  d="M35,72.3c-4.025,0-7.3-3.274-7.3-7.3V35c0-4.025,3.274-7.3,7.3-7.3h30c4.025,0,7.3,3.274,7.3,7.3 v30c0,4.025-3.274,7.3-7.3,7.3H35z"></path>
            <path fill="#472b29"
                  d="M65,28.4c3.639,0,6.6,2.961,6.6,6.6v30c0,3.639-2.961,6.6-6.6,6.6H35c-3.639,0-6.6-2.961-6.6-6.6 V35c0-3.639,2.961-6.6,6.6-6.6H65 M65,27H35c-4.4,0-8,3.6-8,8v30c0,4.4,3.6,8,8,8h30c4.4,0,8-3.6,8-8V35C73,30.6,69.4,27,65,27 L65,27z"></path>
            <g>
                <path fill="#472b29"
                      d="M68.5,47.375c-0.276,0-0.5-0.224-0.5-0.5V43c0-0.276,0.224-0.5,0.5-0.5S69,42.724,69,43v3.875 C69,47.151,68.776,47.375,68.5,47.375z"></path>
            </g>
            <g>
                <path fill="#472b29"
                      d="M68.5,40.5c-0.276,0-0.5-0.224-0.5-0.5v-2c0-0.276,0.224-0.5,0.5-0.5S69,37.724,69,38v2 C69,40.276,68.776,40.5,68.5,40.5z"></path>
            </g>
            <g>
                <path fill="#da914b"
                      d="M62.95,64.449c-0.401,0-0.777-0.155-1.061-0.438l-9.899-9.899c-0.585-0.585-0.585-1.537,0-2.122 c0.283-0.283,0.66-0.438,1.061-0.438s0.777,0.155,1.061,0.438l9.899,9.899c0.585,0.585,0.585,1.537,0,2.122 C63.727,64.294,63.351,64.449,62.95,64.449z"></path>
                <path fill="#472b29"
                      d="M53.05,52.05c0.267,0,0.518,0.104,0.707,0.293l9.9,9.899c0.189,0.189,0.293,0.44,0.293,0.707 s-0.104,0.518-0.293,0.707c-0.189,0.189-0.44,0.293-0.707,0.293s-0.518-0.104-0.707-0.293l-9.9-9.899 c-0.189-0.189-0.293-0.44-0.293-0.707s0.104-0.518,0.293-0.707C52.532,52.154,52.783,52.05,53.05,52.05 M53.05,51.05 c-0.512,0-1.024,0.195-1.414,0.586c-0.781,0.781-0.781,2.047,0,2.828l9.899,9.899c0.391,0.391,0.902,0.586,1.414,0.586 s1.024-0.195,1.414-0.586c0.781-0.781,0.781-2.047,0-2.828l-9.899-9.899C54.074,51.246,53.562,51.05,53.05,51.05L53.05,51.05z"></path>
            </g>
            <g>
                <path fill="#472b29"
                      d="M64,69H36c-2.757,0-5-2.243-5-5V36c0-2.757,2.243-5,5-5h25.375c0.276,0,0.5,0.224,0.5,0.5 s-0.224,0.5-0.5,0.5H36c-2.206,0-4,1.794-4,4v28c0,2.206,1.794,4,4,4h28c2.206,0,4-1.794,4-4V49.625c0-0.276,0.224-0.5,0.5-0.5 s0.5,0.224,0.5,0.5V64C69,66.757,66.757,69,64,69z"></path>
            </g>
            <g>
                <path fill="#fde751" d="M46.5 35.5A11 11 0 1 0 46.5 57.5A11 11 0 1 0 46.5 35.5Z"></path>
                <path fill="#472b29"
                      d="M46.5,36C52.29,36,57,40.71,57,46.5S52.29,57,46.5,57S36,52.29,36,46.5S40.71,36,46.5,36 M46.5,35 C40.149,35,35,40.149,35,46.5S40.149,58,46.5,58S58,52.851,58,46.5S52.851,35,46.5,35L46.5,35z"></path>
            </g>
            <g>
                <path fill="#88cdd7" d="M46.5 38.5A8 8 0 1 0 46.5 54.5A8 8 0 1 0 46.5 38.5Z"></path>
                <path fill="#472b29"
                      d="M46.5,39c4.136,0,7.5,3.364,7.5,7.5S50.636,54,46.5,54S39,50.636,39,46.5S42.364,39,46.5,39 M46.5,38c-4.694,0-8.5,3.806-8.5,8.5s3.806,8.5,8.5,8.5s8.5-3.806,8.5-8.5S51.194,38,46.5,38L46.5,38z"></path>
            </g>
            <g>
                <path fill="#472b29"
                      d="M40.5,46.75c-0.138,0-0.25-0.112-0.25-0.25c0-1.732,0.729-3.401,1.999-4.581 c0.1-0.093,0.259-0.089,0.353,0.014c0.094,0.101,0.088,0.259-0.013,0.354c-1.168,1.084-1.838,2.62-1.838,4.214 C40.75,46.638,40.638,46.75,40.5,46.75z"></path>
            </g>
            <g>
                <path fill="#472b29"
                      d="M43.766,41.407c-0.091,0-0.178-0.05-0.223-0.136c-0.063-0.123-0.014-0.273,0.109-0.337 c0.887-0.454,1.845-0.685,2.848-0.685c0.138,0,0.25,0.112,0.25,0.25s-0.112,0.25-0.25,0.25c-0.923,0-1.804,0.212-2.62,0.63 C43.844,41.398,43.805,41.407,43.766,41.407z"></path>
            </g>
            <g>
                <path fill="#472b29"
                      d="M50.582,51.147c-0.067,0-0.134-0.026-0.183-0.08c-0.094-0.101-0.088-0.259,0.013-0.354 c1.168-1.084,1.838-2.62,1.838-4.214c0-0.138,0.112-0.25,0.25-0.25s0.25,0.112,0.25,0.25c0,1.732-0.729,3.401-1.999,4.581 C50.704,51.125,50.643,51.147,50.582,51.147z"></path>
            </g>
            <g>
                <path fill="#472b29"
                      d="M46.5,52.75c-0.138,0-0.25-0.112-0.25-0.25s0.112-0.25,0.25-0.25c0.923,0,1.804-0.212,2.62-0.63 c0.122-0.06,0.273-0.013,0.336,0.108c0.063,0.123,0.014,0.273-0.109,0.337C48.461,52.52,47.502,52.75,46.5,52.75z"></path>
            </g>
        </svg>
    );
};

export default SearchImageIcon;
