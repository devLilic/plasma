...
const PlaylistArticleSidebar = ({articles, currentId, onSelect, onAddAfter}: PlaylistArticleSidebarProps) => (
    <aside className="ios-card min-h-0 overflow-hidden p-2 lg:sticky lg:top-[104px] lg:h-[calc(100vh-128px)]">
        ...
        <div className="max-h-[420px] space-y-1.5 overflow-y-auto pr-1 lg:max-h-[calc(100%-58px)]">
                ...
                return (
                    <div key={article.id} className="group relative">
                        <button type="button" onClick={() => onSelect(article.id)}
                                className={`flex w-full items-center gap-3 rounded-[17px] border p-2 text-left transition ${active ? 'border-[#2878ff]/25 bg-[#2878ff]/[0.07] shadow-[inset_0_1px_0_rgba(255,255,255,.8)]' : 'border-transparent hover:bg-white/55'}`}>
                            {article.image ? (
                            ...
                            <span className="self-start pt-1 text-[10px] font-semibold tabular-nums text-[#9aa8bc]">{article.playlist_order}</span>
                        </button>
